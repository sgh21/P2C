...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['TotalLoss', 'ShapeReconLoss', 'ShapeCompletionLoss', 'LatentLoss', 'NCCLoss', 'RopeSmoothLoss'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss, shape_recon_loss, shape_compl_loss, latent_recon_loss, ncc_loss ,rope_smooth_loss= base_model.module.get_loss(partial)
            loss.backward()
            ...
            if args.distributed:
                ...
                rope_smooth_loss = dist_utils.reduce_tensor(rope_smooth_loss.detach(), args)
                losses.update([loss.item(), shape_recon_loss.item(), shape_compl_loss.item(), latent_recon_loss.item(), ncc_loss.item(), rope_smooth_loss.item()])
            else:
                losses.update([loss.item(), shape_recon_loss.item(), shape_compl_loss.item(), latent_recon_loss.item(), ncc_loss.item(), rope_smooth_loss.item()])
