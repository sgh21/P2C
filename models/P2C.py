...
class RopeSmoothLoss(nn.Module):
    def __init__(self,group_nums=30,group_size=100,m=0.9):
        super().__init__()
        self._group_nums = group_nums # 子组的数量
        self._group_size = group_size # 子组的大小
        self._m = m #  J=m*|d1-d2|+(1-m)*|a1-a2|
...
class P2C(nn.Module):
    ...
    def _get_lossfnc_and_weights(self, config):
        ...
        self.rope_smooth_loss = RopeSmoothLoss(group_nums=30,group_size=100,m=0.9)
        # self.rope_smooth_loss = RopeSmoothLoss(group_nums=config.group_nums,group_size=config.group_size,m=config.m)
        ...
        self.rope_smooth_weight = config.rope_smooth_weight
    ...
    def get_loss(self, pts):
        ...
        # rope smooth loss
        rope_smooth_loss = self.rope_smooth_weight*self.rope_smooth_loss(pred)
        total_loss = shape_recon_loss + shape_matching_loss + latent_recon_loss + manifold_penalty+rope_smooth_loss
        # print('rope_smooth_loss:',rope_smooth_loss)
        return total_loss, shape_recon_loss, shape_matching_loss, latent_recon_loss, manifold_penalty,rope_smooth_loss
